				...
				<div class="box-header with-border">
						...
						<div class="SearchFilters searchFiltersHorizontal animated fadeIn Hidden" style="margin-bottom:10px;">
								...
								<div class="form-inline" id="SearchFieldsForm">
										...
										{!! Form::open( [ 'id' => 'CoreSearcherForm', 'route' => 'student_category_list', 'method' => 'GET', 'role' => 'search' ] )  !!}
												...
												<div class="row">
														...
														<div class="input-group col-lg-3 col-md-3 col-sm-5 col-xs-11" style="margin:2px;">
																...
																<input id="category_id" name="category_id" value="{{ app( 'request' )->input( 'category_id' ) }}" class="form-control" validateonlynumbers="Ingrese únicamente números." placeholder="ID de Categoría" type="text">
														...
														<div class="input-group col-lg-3 col-md-3 col-sm-5 col-xs-11" style="margin:2px;">
																...
																<input id="description" name="description" value="{{ app( 'request' )->input( 'description' ) }}" class="form-control" placeholder="Descripción" type="text">
						...
						<!-- Delete Selected -->
						<button type="button" aria-label="Eliminar Seleccionados" msgok="¡Las categorías han sido eliminadas con éxito!" msgerror="Hubo un error al intentar eliminar las categorías." msgquestion="¿Desea eliminar las categorías seleccionadas?" class="btn bg-red animated fadeIn NewElementButton Hidden DeleteSelectedElements hint--bottom hint--bounce hint--error"><i class="fa fa-trash-o"></i></button>
						<!-- Activate Selected -->
						<button type="button" aria-label="Activar Seleccionados" msgok="¡Las categorías han sido activadas con éxito!" msgerror="Hubo un error al intentar activar las categorías." msgquestion="¿Desea activar las categorías seleccionadas?" class="btn btn-green animated fadeIn NewElementButton Hidden ActivateSelectedElements hint--bottom hint--bounce hint--success"><i class="fa fa-check-circle"></i></button>
						<!-- Expand Selected -->
												...
												@if( $results->count() < 1 )
														...
														<div class="callout callout-info">
																...
																<h4>
																		...
																		<i class="icon fa fa-info-circle"></i> No se encontraron categorías.

																</h4>

																<p>Puede crear un menú haciendo <a href="{{ route( 'student_category_create' ) }}">click aqui</a>.</p>
																...
																<div class="form-group col-lg-3 col-md-4 col-sm-4 list-col-container">
																		...
																		<div class="col-lg-11 col-md-10 col-sm-10 col-xs-10">
																				<div class="listRowInner">
																						<img class="img-circle hideMobile" src="/views/students/categories/images/default/default.png" alt="{{ $element->name }}">
																		...
																		<div class="listRowInner">
																				...
																				<span class="listTextStrong">
																						@if( $element->description )
																								...
																								<span class="label label-brown">{{ $element->description }}</span>

																						@else

																								<span class="label label-default">Sin Descripción</span>
																...
																<div class="listActions flex-justify-center Hidden">
																		...
																		<div>
																				...
																				<span class="roundItemActionsGroup">
																						...
																						@if( auth()->user()->hasRoute( 'student_category_show' ) )
																						...
																						@if( auth()->user()->hasRoute( 'student_category_edit' ) )
																						...
																						@if( $element->status == 'A' && auth()->user()->hasRoute( 'student_category_delete' ) )

																								<!-- Delete Button -->
																								<a class="deleteElement hint--bottom hint--bounce hint--error" aria-label="Eliminar" url="/alumnos/categorias/eliminar/{{ $element->id }}" msgok="El perfil <b>{{$element->name}}</b> ha sido eliminada correctamente." msgerror="Hubo un error al intentar eliminar la categoría <b>{{$element->name}}</b>." msgquestion="¿Desea eliminar la categoría <b>{{$element->name}}</b>?" id="delete_{{$element->id}}">
																						...
																						@if( $element->status == 'I' && auth()->user()->hasRoute( 'student_category_activate' ) )

																								<!-- Activate Button -->
																								<a class="activateElement hint--bottom hint--bounce hint--success" aria-label="Activar" url="/alumnos/categorias/activar/{{ $element->id }}" msgok="El perfil <b>{{ $element->name }}</b> ha sido activada." msgerror="Hubo un error al intentar activar la categoría <b>{{ $element->name }}</b>." msgquestion="¿Desea activar la categoría <b>{{ $element->name }}</b>?" id="activate_{{ $element->id }}">
