...
@section( 'tab_title', 'Listado de Middlewares' )

@section( 'page_title', 'Listado' )

@section( 'page_subtitle', 'Middlewares' )
				...
				<div class="box-header with-border">
						...
						<div class="SearchFilters searchFiltersHorizontal animated fadeIn Hidden" style="margin-bottom:10px;">
								...
								<div class="form-inline" id="SearchFieldsForm">
										...
										{!! Form::open( [ 'id' => 'CoreSearcherForm', 'route' => 'middleware_list', 'method' => 'GET', 'role' => 'search' ] )  !!}
												...
												<div class="row">
														...
														<div class="input-group col-lg-3 col-md-3 col-sm-5 col-xs-11" style="margin:2px;">
																...
																<input id="middleware_id" name="middleware_id" value="{{ app( 'request' )->input( 'middleware_id' ) }}" class="form-control" validateonlynumbers="Ingrese únicamente números." placeholder="ID de Middleware" type="text" tabindex="1">
														...
														<div class="input-group col-lg-3 col-md-3 col-sm-5 col-xs-11" style="margin:2px;">
																...
																@if( app( 'request' )->input( 'view_order_field' ) == 'class' )

																		@if( app( 'request' )->input( 'view_order_mode' ) == 'desc' )

																				<span class="input-group-addon order-arrows sort-activated hint--bottom-right hint--bounce" order="class" mode="desc" aria-label="Ordenar de Menor a Mayor"><i class="fa fa-sort-alpha-desc"></i></span>

																		@else

																				<span class="input-group-addon order-arrows sort-activated hint--bottom-right hint--bounce" order="class" mode="asc" aria-label="Ordenar de Mayor a Menor"><i class="fa fa-sort-alpha-asc"></i></span>

																		@endif

																@else

																		<span class="input-group-addon order-arrows hint--bottom-right hint--bounce" order="class" mode="desc" aria-label="Ordenar de Menor a Mayor"><i class="fa fa-sort-alpha-asc"></i></span>

																@endif

																<input id="class" name="class" value="{{ app( 'request' )->input( 'class' ) }}" class="form-control" placeholder="Clase" type="text" tabindex="1">
														...
														<div class="input-group col-lg-3 col-md-3 col-sm-5 col-xs-11" style="margin:2px;">
																...
																<input id="description" name="description" value="{{ app( 'request' )->input( 'description' ) }}" class="form-control" placeholder="Descripción" type="text" tabindex="2">
						...
						<!-- Delete Selected -->
						<button type="button" aria-label="Eliminar Seleccionados" msgok="¡Los middlewares han sido eliminados con éxito!" msgerror="Hubo un error al intentar eliminar los middlewares." msgquestion="¿Desea eliminar los middlewares seleccionadas?" class="btn bg-red animated fadeIn NewElementButton Hidden DeleteSelectedElements hint--bottom hint--bounce hint--error"><i class="fa fa-trash-o"></i></button>
						<!-- Activate Selected -->
						<button type="button" aria-label="Activar Seleccionados" msgok="¡Los middlewares han sido activados con éxito!" msgerror="Hubo un error al intentar activar los middlewares." msgquestion="¿Desea activar los middlewares seleccionadas?" class="btn btn-green animated fadeIn NewElementButton Hidden ActivateSelectedElements hint--bottom hint--bounce hint--success"><i class="fa fa-check-circle"></i></button>
						<!-- Expand Selected -->
												...
												@if( $results->count() < 1 )
														...
														<div class="callout callout-info bg-aqua-gradient">
																...
																<h4>
																		...
																		<i class="icon fa fa-info-circle"></i> No se encontraron middlewares.
												...
												@foreach( $results as $element )
														...
														<div class="row listRow {{$rowColor}} " id="row_{{ $element->id }}">
																...
																<div class="form-group col-lg-3 col-md-4 col-sm-4 list-col-container">
																		...
																		<div class="col-lg-11 col-md-10 col-sm-10 col-xs-10">
																				<div class="listRowInner">
																						<img class="img-circle hideMobile" src="/views/middlewares/images/default/default.png" alt="{{ $element->name }}">
																...
																<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
																		<div class="listRowInner">
																				<span class="smallTitle">Clase</span>
																				<span class="listTextStrong">
																						<span class="badge bg-purple">{{ $element->class }}</span>
																				</span>
																		</div>
																</div>
																<!-- /Col -->
																...
																<div class="listActions flex-justify-center Hidden">
																		...
																		<div>
																				...
																				<span class="roundItemActionsGroup">
																						...
																						<!-- Show Button -->
																						<a class="hint--bottom hint--bounce" aria-label="Ver Detalle" href="/middlewares/{{$element->id}}" id="view_{{$element->id}}">
																								<button type="button" class="btn btn-github"><i class="fa fa-eye"></i></button>
																						</a>

																						<!-- Edit Button -->
																						<a href="/middlewares/editar/{{$element->id}}/" class="hint--bottom hint--bounce hint--info" aria-label="Editar">
																						...
																						@if( $element->status == 'A' )
																								...
																								<!-- Delete Button -->
																								<a class="deleteElement hint--bottom hint--bounce hint--error" aria-label="Eliminar" url="/middlewares/eliminar/{{ $element->id }}" msgok="El middleware <b>{{$element->name}}</b> ha sido eliminada correctamente." msgerror="Hubo un error al intentar eliminar el middleware <b>{{$element->name}}</b>." msgquestion="¿Desea eliminar el middleware <b>{{$element->name}}</b>?" id="delete_{{$element->id}}">
																						...
																						@if( $element->status == 'I' )
																								...
																								<!-- Activate Button -->
																								<a class="activateElement hint--bottom hint--bounce hint--success" aria-label="Activar" url="/middlewares/activar/{{ $element->id }}" msgok="El middleware <b>{{ $element->name }}</b> ha sido activada." msgerror="Hubo un error al intentar activar el middleware <b>{{ $element->name }}</b>." msgquestion="¿Desea activar el middleware <b>{{ $element->name }}</b>?" id="activate_{{ $element->id }}">
