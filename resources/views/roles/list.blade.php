...
@section( 'tab_title', 'Listado de Perfiles' )

@section( 'page_title', 'Listado' )

@section( 'page_subtitle', 'Perfiles' )
				...
				<div class="box-header with-border">
						...
						<div class="SearchFilters searchFiltersHorizontal animated fadeIn Hidden" style="margin-bottom:10px;">
								...
								<div class="form-inline" id="SearchFieldsForm">
										...
										{!! Form::open( [ 'id' => 'CoreSearcherForm', 'route' => 'role_list', 'method' => 'GET', 'role' => 'search' ] )  !!}
												...
												<div class="row">
														...
														<div class="input-group col-lg-3 col-md-3 col-sm-5 col-xs-11" style="margin:2px;">
																...
																<input id="role_id" name="role_id" value="{{ app( 'request' )->input( 'role_id' ) }}" class="form-control" validateonlynumbers="Ingrese únicamente números." placeholder="ID de Perfil" type="text">
						...
						<!-- Delete Selected -->
						<button type="button" aria-label="Eliminar Seleccionados" msgok="¡Los perfiles han sido eliminados con éxito!" msgerror="Hubo un error al intentar eliminar los perfiles." msgquestion="¿Desea eliminar los perfiles seleccionados?" class="btn bg-red animated fadeIn NewElementButton Hidden DeleteSelectedElements hint--bottom hint--bounce hint--error"><i class="fa fa-trash-o"></i></button>
						<!-- Activate Selected -->
						<button type="button" aria-label="Activar Seleccionados" msgok="¡Los perfiles han sido activados con éxito!" msgerror="Hubo un error al intentar activar los perfiles." msgquestion="¿Desea activar los perfiles seleccionados?" class="btn btn-green animated fadeIn NewElementButton Hidden ActivateSelectedElements hint--bottom hint--bounce hint--success"><i class="fa fa-check-circle"></i></button>
						<!-- Expand Selected -->
												...
												@if( $results->count() < 1 )
														...
														<div class="callout callout-info">
																...
																<h4>
																		...
																		<i class="icon fa fa-info-circle"></i> No se encontraron perfiles.

																</h4>

																<p>Puede crear un menú haciendo <a href="{{ route( 'role_create' ) }}">click aqui</a>.</p>
																...
																<div class="form-group col-lg-3 col-md-4 col-sm-4 list-col-container">
																		...
																		<div class="col-lg-11 col-md-10 col-sm-10 col-xs-10">
																				<div class="listRowInner">
																						<img class="img-circle hideMobile" src="/views/roles/images/default/default.png" alt="{{ $element->name }}">
																...
																<div class="listActions flex-justify-center Hidden">
																		...
																		<div>
																				...
																				<span class="roundItemActionsGroup">
																						...
																						<!-- Show Button -->
																						<a class="hint--bottom hint--bounce" aria-label="Ver Detalle" href="/perfiles/{{$element->id}}" id="view_{{$element->id}}">
																								<button type="button" class="btn btn-github"><i class="fa fa-eye"></i></button>
																						</a>

																						<!-- Edit Button -->
																						<a href="/perfiles/editar/{{$element->id}}/" class="hint--bottom hint--bounce hint--info" aria-label="Editar">
																						...
																						@if( $element->status == 'A' )
																								...
																								<!-- Delete Button -->
																								<a class="deleteElement hint--bottom hint--bounce hint--error" aria-label="Eliminar" url="/perfiles/eliminar/{{ $element->id }}" msgok="El perfil <b>{{$element->name}}</b> ha sido eliminado correctamente." msgerror="Hubo un error al intentar eliminar el perfil <b>{{$element->name}}</b>." msgquestion="¿Desea eliminar el perfil <b>{{$element->name}}</b>?" id="delete_{{$element->id}}">
																						...
																						@if( $element->status == 'I' )
																								...
																								<!-- Activate Button -->
																								<a class="activateElement hint--bottom hint--bounce hint--success" aria-label="Activar" url="/perfiles/activar/{{ $element->id }}" msgok="El perfil <b>{{ $element->name }}</b> ha sido activado." msgerror="Hubo un error al intentar activar el perfil <b>{{ $element->name }}</b>." msgquestion="¿Desea activar el perfil <b>{{ $element->name }}</b>?" id="activate_{{ $element->id }}">
